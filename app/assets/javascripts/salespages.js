$(function(){
  $('.category_parent').on('change',function(e){
    ...
    if (val !== ""){
      $.ajax({
        ...
        type: 'GET',
        dataType: 'json',
        url: '/salepages/ancestry_children'
      })
      .done(function(datas){
  ...
  $(document).on('change','.category__children',function(e){
    ...
    function appendGrandChildSelect(datas){
      $('.category__grand_children').remove();
      var select = `<select class="main__content__name__text category__grand_children" name="item[category_id]" id="item_category_id">
                  </select>`
      $('.category_area').append(select);
      var options = `<option value="">選択してください</option>`
      datas.forEach(function(data){
    ...
    if (val !== ""){
      $.ajax({
        ...
        type: 'GET',
        dataType: 'json',
        url: '/salepages/ancestry_grand_children'
      })
      .done(function(datas){
